...
export async function createTodo(params: CreateTodoParams) {
  ...
  try {
    ...
    if (params.agent_id) message += `\nAgent: ${params.agent_id}`;
    if (params.phase) message += `\nPhase: ${params.phase}`;
...
export async function updateTodo(params: UpdateTodoParams) {
  ...
  try {
    ...
    if (params.agent_id !== undefined) message += `\nAssignee: ${params.agent_id || '(unassigned)'}`;
    if (params.parent_id !== undefined) message += `\nParent: ${params.parent_id || '(none)'}`;
    if (params.progress !== undefined) message += `\nProgress: ${params.progress}%`;
    if (params.phase) message += `\nPhase: ${params.phase}`;
    if (params.estimated_hours !== undefined) message += `\nEstimated: ${params.estimated_hours}h`;
    if (params.actual_hours !== undefined) message += `\nActual: ${params.actual_hours}h`;
...
export async function getTodos(params: GetTodosParams = {}) {
  ...
  try {
    ...
    const todos = db.prepare(query).all(...values) as Todo[];
    ...
    todos.forEach((todo, index) => {
      ...
      message += `   ID: ${todo.id}`;
      if (todo.agent_id) message += ` | Agent: ${todo.agent_id}`;
      if (todo.priority) message += ` | Priority: ${todo.priority}`;
 ...
 * TODO完了
 */
export async function completeTodo(params: CompleteTodoParams) {
